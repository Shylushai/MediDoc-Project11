...
def index():
    ...
    if current_user.role == "admin":
        ...
        for reminder in reminders:
            formatted_date = datetime.strptime(reminder["date"], "%Y-%m-%d").strftime(
                "%d-%b-%Y"
            )
            reminder_data = {
...
def admin_setup():
        ...
        try:
            ...
            if "doctor_name" in request.form:
                ...
                if doctor_name and specialty and contact:
                    logging.debug(
                        f"Adding doctor: {doctor_name}, {specialty}, {contact}"
                    )
                    execute_query(
                        "INSERT INTO Doctors (name, specialty, contact, department_id) VALUES (?, ?, ?, ?)",
            ...
            if "time_slots_select" in request.form:
                ...
                for slot in selected_slots:
                    execute_query(
                        "INSERT INTO AvailableTimeSlots (time) VALUES (?)", (slot,)
                    )
    ...
    return render_template(
        "admin_setup.html", suggested_times=suggested_times, departments=departments
    )
...
def book_appointment():
    if request.method == "POST":
        ...
        reason = (
            request.form["reason"] if request.form["reason"] else "No message provided"
        )
        execute_query(
...
def update_patient(patient_id):
    if current_user.role not in ["patient", "admin", "doctor", "receptionist"]:
        return redirect(url_for("unorthorised"))

    if request.method == "POST" and current_user.role == "patient":
        # Patient information
    ...
    if current_user.role == "patient":
        patient = fetch_query(
            "SELECT * FROM Patients WHERE user_id = ?", (current_user.id,)
        )
        if patient:
            patient = patient[0]
            print(patient)
            emergency_contact = fetch_query(
                "SELECT * FROM EmergencyContact WHERE patient_id = ?",
                (patient["patient_id"],),
            )
            emergency_contact = emergency_contact[0]
    if current_user.role == "admin":
        patient = fetch_query("SELECT * FROM Patients WHERE user_id = ?", (patient_id,))
        if patient:
            patient = patient[0]
            print(patient)
            emergency_contact = fetch_query(
    ...
    return render_template(
        "update_patient.html", patient=patient, emergency_contact=emergency_contact
    )
...
def receptionist_book_appointment():
    ...
    available_slots = fetch_query("SELECT * FROM AvailableTimeSlots")
    return render_template(
        "receptionist_book_appointment.html",
        doctors=doctors,
        available_slots=available_slots,
    )
...
def search_patient():
    ...
    if request.method == "POST" and "search_term" in request.form:
        ...
        execute_query(
            "INSERT INTO SearchHistory (receptionist_id, patient_name) VALUES (?, ?)",
            (current_user.id, search_term),
        )

    # Fetch search history
    search_history = fetch_query(
        "SELECT * FROM SearchHistory WHERE receptionist_id = ? ORDER BY search_time DESC",
        (current_user.id,),
    )

    return render_template(
        "search_patient.html", patients=patients, search_history=search_history
    )
...
def clear_search_history():
    ...
    execute_query(
        "DELETE FROM SearchHistory WHERE receptionist_id = ?", (current_user.id,)
    )
    return redirect(url_for("search_patient"))
...
def upload():
    ...
    return render_template(
        "Doctor_patient_search.html", users=users, files=files, patients=patients
    )
...
def document_download(id, file_name):
    ...
    return send_from_directory(
        "uploads/{value}".format(value=id), file, as_attachment=True
    )
...
def update_reminder_seen_status(patient_id):
    ...
    query = (
        "UPDATE Reminders SET seen_in_ui = 1 WHERE patient_id = ? AND seen_in_ui = 0"
    )
...
def confirm_appointment():
    ...
    if request.method == "POST":
        ...
        execute_query(
            "UPDATE Appointments SET status = 'Confirmed' WHERE id = {value}".format(
                value=id
            )
        )
        print("works")
